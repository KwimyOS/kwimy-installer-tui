...
pub fn config() -> &'static Config {
    CONFIG.get_or_init(|| {
        ...
        let parsed: Config = toml::from_str(raw).expect("Invalid kwimy-installer-tui config.toml");
        validate_config(&parsed).expect("Invalid kwimy-installer-tui config.toml");
        parsed
    })
}
